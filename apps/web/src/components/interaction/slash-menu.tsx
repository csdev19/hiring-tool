...
interface UseSlashMenuArgs {
  ...
  /** Position override for the floating menu (defaults to above the area) */
...
export function useSlashMenu({
  ...
  const pick = useCallback(
    ...
    [insertSnippet, slashAt, close, textareaRef, value.length],
  );
  ...
  const menu =
    open && items.length > 0 ? (
      <div
        className={cn(
          "absolute bottom-full left-0 z-20 mb-1 w-[256px] rounded-md border border-border bg-surface-2 py-1 shadow-md",
          menuClassName,
        )}
      >
        {items.map((item, i) => (
          ...
          >
            ...
            <span className="mono text-[11px] text-text-muted">{item.insert().trim()}</span>
          </button>
        ))}
      </div>
    ) : null;
