...
export const hiringProcessBaseSchema = z.object({
  ...
  status: z.enum(['ongoing', 'rejected', 'dropped-out', 'hired']),
...
// Derived schemas
export const createHiringProcessSchema = hiringProcessBaseSchema.pick({
  companyName: true,
  status: true,
}).extend({
...
// Type inference
export type HiringProcess = z.infer<typeof hiringProcessBaseSchema>;
...
**Example Domains**:
- Job application tracker (hiring-tool)
...
### Hiring Tool: Schema-Based ✅

**Rationale:**
- Primarily a tracking/logging application
- Simple state transitions (ongoing → rejected/hired)
...
export function canTransitionStatus(
  ...
  const allowedTransitions = {
    'ongoing': ['rejected', 'dropped-out', 'hired'],
    'rejected': [],
    'dropped-out': [],
    'hired': [],
  };
...
export function calculateDaysSinceLastUpdate(
  ...
  const now = new Date();
  const updated = new Date(process.updatedAt);
  return Math.floor((now.getTime() - updated.getTime()) / (1000 * 60 * 60 * 24));
}

export function isStale(process: HiringProcess): boolean {
  return calculateDaysSinceLastUpdate(process) > 30;
}
```
...
```typescript
// packages/domain/src/value-objects/interview-status.ts
export class InterviewStatusVO {
  private constructor(private readonly value: InterviewStatus) {}
  
  public static create(value: string): InterviewStatusVO {
    if (!isValidStatus(value)) {
      throw new Error(`Invalid status: ${value}`);
    }
    return new InterviewStatusVO(value as InterviewStatus);
  }
  
  public canTransitionTo(target: InterviewStatus): boolean {
    // Encapsulate business logic
  }
  
  public getValue(): InterviewStatus {
    return this.value;
  }
}
```
...
```typescript
// packages/domain/src/entities/hiring-process.ts
export class HiringProcess {
  private readonly _id: UniqueIdVO;
  private readonly _status: InterviewStatusVO;
  private readonly _companyName: string;
  
  private constructor(props: HiringProcessProps) {
    this._id = UniqueIdVO.create(props.id);
    this._status = InterviewStatusVO.create(props.status);
    this._companyName = props.companyName;
  }
  
  public static create(props: HiringProcessProps): HiringProcess {
    const validated = hiringProcessBaseSchema.parse(props);
    return new HiringProcess(validated);
  }
  
  public reject(reason?: string): HiringProcess {
...
- ⚠️ Business stakeholders requesting more complex features
...
**For hiring-tool**, we're using the **Schema-Based approach** because:
