...
```typescript
import { tryCatch } from "@interviews-tool/domain/types";
...
```typescript
import { isFailure, isSuccess } from "@interviews-tool/domain/types";
...
3. **Use with error handlers**: Combine with [Error Handlers](/docs/backend/error-handlers) for consistent error handling.
...
- [Error Handlers](/docs/backend/error-handlers) - Utilities for handling Result types
- [Promise Handlers](/docs/backend/promise-handlers) - Convenience wrappers for promises
- [Response Helpers](/docs/backend/response-helpers) - Creating API responses
