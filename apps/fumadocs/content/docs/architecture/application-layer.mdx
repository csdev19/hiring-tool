...
1. **Mobile safety is structural.** The mobile app imports `@interviews-tool/domain` — a package that _cannot_ contain infrastructure by design. With vertical slicing, the mobile app would need to avoid `@interviews-tool/hiring/application` — a subpath in the same package. One wrong import and Metro bundles your DB code into the mobile binary.
...
4. **Cross-context sharing is trivial.** If evaluations need a type from hiring, it's a relative import within the same package.
...
### `@interviews-tool/domain` — Pure, Mobile-Safe
...
- **Repository interfaces** — `IHiringProcessRepository` (just TypeScript interfaces, erased at runtime)
...
### `@interviews-tool/application` — Server-Only Use Cases
...
Example — `createHiringProcess`:

```typescript
// packages/application/src/hiring/create-hiring-process.ts
...
### `@interviews-tool/infra-db` — Infrastructure

Contains concrete implementations:

- **Drizzle schema definitions** — table columns, relations
- **Repository implementations** — `HiringProcessRepository implements IHiringProcessRepository`
...
.post("/", async ({ body, hiringProcessRepo, user }) => {
...
```typescript
// apps/mobile/app/hiring/create.tsx
...
const validated = createHiringProcessSchema.parse(formData);
const response = await fetch(`${API_URL}/hiring-processes`, {
  method: "POST",
...
Zod schemas in `@interviews-tool/domain/schemas` act as the **contract** between consumers and the application layer:

```
Mobile app                                 Server
    |                                        |
    |  createHiringProcessSchema.parse()     |  createHiringProcessSchema (validation)
    |  (client-side form validation)         |  (Elysia body validation)
    |                                        |
    |  ---------- HTTP POST body ----------> |
    |                                        |
    |                              CreateHiringProcess (type)
    |                                        |
    |                              createHiringProcess(input)
    |                                        |
    |  <-------- HiringProcessBase --------- |
    |  (ApiResponse<HiringProcessBase>)      |
```

- **Input contract:** `createHiringProcessSchema` (Zod) / `CreateHiringProcess` (type)
- **Output contract:** `HiringProcessBase` (type inferred from `hiringProcessBaseSchema`)
...
### Safe to import in mobile (`@interviews-tool/domain`)
...
- `@interviews-tool/infra-db` — DB client, connection strings, Drizzle schemas
- `@interviews-tool/application` — use cases that reference domain interfaces for repos
- `@interviews-tool/infra-auth` server config — server-side auth secrets
...
| -------------- | ------------------------------ | :----------: | ---------------------------------------- |
| Domain         | `@interviews-tool/domain`      |     Yes      | Constants, schemas, types, interfaces    |
| Application    | `@interviews-tool/application` |      No      | Use cases, business logic orchestration  |
| Infrastructure | `@interviews-tool/infra-db`    |      No      | Drizzle repos, mappers, DB client        |
