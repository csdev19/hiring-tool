...
Application code that uses the **concrete class** (e.g. `HiringProcessRepository` from `@interviews-tool/infra-db/repositories`) does not need to change; only the interface name in the domain and the implementation’s `implements` clause change.
