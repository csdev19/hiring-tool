...
```text
HiringProcessRepositoryContract
```

Why:

- Makes it explicit this is not an implementation
- Avoids language-specific prefixes like `I` (e.g. `IHiringProcessRepository`)
...
packages/
  domain/
    src/
      repositories/
        hiring-process.repository.ts   # Contract(s) + PaginatedResult, etc.
```
...
```typescript
/**
 * Hiring process repository contract (domain layer).
 * Defines the data access API only; no implementation lives here.
 */
export interface HiringProcessRepositoryContract {
...
- **`HiringProcessRepository`** – current Drizzle implementation (concrete class name)
- **`DrizzleHiringProcessRepository`** – alternative name that makes the technology obvious
...
packages/
  db/
    src/
      ...
      mappers/
        hiring-process.mapper.ts       # DB row ↔ domain type mapping
```
...
export class HiringProcessRepository implements HiringProcessRepositoryContract {
...
export class GetHiringProcesses {
  constructor(
    private readonly repository: HiringProcessRepositoryContract
  ) {}
...
| Idea                                          | Role                                                                                    |
| --------------------------------------------- | --------------------------------------------------------------------------------------- |
| **Domain-Driven Design (DDD)**                | Domain owns business rules and contracts; bounded contexts are explicit (e.g. hiring).  |
...
| Implementations live in infrastructure (e.g. `packages/db`) | “How” is isolated and replaceable.                                              |
...
If you have existing repository interfaces named with an `I` prefix (e.g. `IHiringProcessRepository`), you can rename them to the contract naming as follows.

1. **Domain** (`packages/domain/src/repositories/<entity>.repository.ts`): Rename the interface to `<Entity>RepositoryContract` and update the JSDoc to state it is the contract only (no implementation). Example: `IHiringProcessRepository` → `HiringProcessRepositoryContract`.

2. **DB package** (`packages/db/src/repositories/<entity>.repository.ts`): Update the import from the domain to use the new contract name, and change the class `implements` clause and class JSDoc to reference the new name.

3. **Docs**: In `apps/fumadocs/content/docs/architecture/*.mdx`, replace any remaining references to the old interface name (e.g. `IHiringProcessRepository`) with the new one (`HiringProcessRepositoryContract`).

Application code that uses the **concrete class** (e.g. `HiringProcessRepository` from `@interviews-tool/db/repositories`) does not need to change; only the interface name in the domain and the implementation’s `implements` clause change.

---

## Related Documentation

- [Repository Pattern](/docs/architecture/repository-pattern) – Detailed implementation guide and examples
- [Repository Refactoring Summary](/docs/architecture/repository-refactoring-summary) – Refactoring history
- [Application Services Layer](/docs/architecture/application-services-layer) – How use cases use repositories
