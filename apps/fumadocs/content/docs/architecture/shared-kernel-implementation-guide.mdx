...
- Existing `@hiring-tool/domain` package
...
packages/domain/
  src/
    constants/
      currency.ts              # ← Move to shared-kernel
      hiring-process-status.ts # ← Keep
      interaction-type.ts      # ← Keep
    types/
      api-response.ts          # ← Move to shared-kernel
      result.ts                # ← Move to shared-kernel
```
...
packages/shared-kernel/
  src/
    api.ts                     # ← From api-response.ts
    currency.ts                # ← From currency.ts
    result.ts                  # ← From result.ts
    utils.ts                   # ← New file with ObjectProperties
    index.ts                   # ← Barrel exports

packages/domain/
  src/
    constants/
      hiring-process-status.ts # ← Stays
      interaction-type.ts      # ← Stays
      salary-rate-type.ts      # ← Stays
```
...
{
  "extends": "@hiring-tool/config/tsconfig.base.json",
  "compilerOptions": {
    "outDir": "dist",
    "rootDir": "src"
  },
...
export type {
  ...
  Failure,
  ObjectProperties,
} from "@hiring-tool/shared-kernel";

export { tryCatch, isSuccess, isFailure, unwrap } from "@hiring-tool/shared-kernel";
```
...
```typescript
// Before
import { CURRENCIES } from "@hiring-tool/domain/constants";
import { Result } from "@hiring-tool/domain/types";
import { ApiResponse } from "@hiring-tool/domain/types";

// After
import { CURRENCIES } from "@hiring-tool/shared-kernel/currency";
...
import { CURRENCIES, Result, ApiResponse } from "@hiring-tool/domain";
```
...
Add `@hiring-tool/shared-kernel` to all packages that need it:
...
2. Check `node_modules/@hiring-tool/shared-kernel` exists
