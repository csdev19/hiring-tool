...
```typescript
// ✅ Good: Using the const object directly with z.enum()
import { z } from "zod";
import { HIRING_PROCESS_STATUSES } from "@interviews-tool/domain/constants";

export const hiringProcessSchema = z.object({
  status: z.enum(HIRING_PROCESS_STATUSES),
});

// ✅ Good: For optional status fields
export const filterSchema = z.object({
  status: z.enum(HIRING_PROCESS_STATUSES).optional(),
});
```

**Important:** According to [Zod's enum documentation](https://zod.dev/api#enum), `z.enum()` accepts enum-like object literals (`{ [key: string]: string | number }`). Use the const object (`HIRING_PROCESS_STATUSES`) directly with `z.enum()`. The constant object is the single source of truth.
...
- `HIRING_PROCESS_STATUS_INFO` - Display metadata (label, description, order, category)
