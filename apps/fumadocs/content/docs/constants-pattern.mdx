...
**Important:** Always use the const object (e.g., `INTERACTION_TYPES`) directly with `z.enum()`, not the `*_VALUES` array. This is:
...
### Hiring Process Status (`packages/domain/src/constants/hiring-process-status.ts`)

- `HIRING_PROCESS_STATUSES` - Status values (first-contact, ongoing, on-hold, rejected, dropped-out, hired)
- `HiringProcessStatus` - Type for status values
- `HIRING_PROCESS_STATUS_VALUES` - Array of all statuses (deprecated, use `Object.values(HIRING_PROCESS_STATUSES)`)
- `isValidHiringProcessStatus()` - Validation function
- `HIRING_PROCESS_STATUS_INFO` - Display metadata (label, description, order, category, color)
- `STATUS_TRANSITIONS` - Valid status transition rules

#### Cross-Platform Status Colors

The `color` property in `HIRING_PROCESS_STATUS_INFO` provides a single source of truth for status colors across both the web and mobile apps. Each hex value corresponds to the Tailwind color-500 family used by the web app:

| Status         | Color Family | Hex       |
| -------------- | ------------ | --------- |
| first-contact  | purple       | `#8b5cf6` |
| ongoing        | blue         | `#3b82f6` |
| on-hold        | yellow       | `#f59e0b` |
| rejected       | red          | `#ef4444` |
| dropped-out    | gray         | `#6b7280` |
| hired          | green        | `#22c55e` |
| offer-made     | cyan         | `#06b6d4` |
| offer-accepted | emerald      | `#10b981` |
...
// Mobile usage
const statusInfo = HIRING_PROCESS_STATUS_INFO[status as HiringProcessStatus];
const backgroundColor = statusInfo.color;  // e.g. "#8b5cf6"
const label = statusInfo.label;            // e.g. "First Contact"
```
...
2. **Use the type, not the union**
   ...
   // ✅ Good
   import type { InterviewStatus } from "@interviews-tool/infra-db";
   ```
